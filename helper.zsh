...
function pmodload {
  ...
  fpath=(${pmodules:+${PDIR}/modules/${^pmodules}/functions(/FN)} $fpath)
  ...
  for pmodule in "$pmodules[@]"; do
    if zstyle -t ":prezto:module:$pmodule" loaded; then
      continue
    elif [[ ! -d "$PDIR/modules/$pmodule" ]]; then
      print "$0: no such module: $pmodule" >&2
      continue
    else
      if [[ -s "$PDIR/modules/$pmodule/init.zsh" ]]; then
        source "$PDIR/modules/$pmodule/init.zsh"
      fi
      ...
      if (( $? == 0 )); then
        ...
        fpath[(r)$PDIR/modules/${pmodule}/functions]=()
