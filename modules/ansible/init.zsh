...
# Don't check for ansible, we might not have it (yet)
...
# security add-generic-password -a $USER -s awx_token -w SomeAPItoken login.keychain
	if is-darwin; then
		export CONTROLLER_TOKEN=$(security find-generic-password -a $USER -s awx_token -w login.keychain)
	fi
fi

# Similarly, ARA should be handled by direnv now
